# AGENT 6.95 "MOMENTUM PRIME" — SINGLE-FILE SOVEREIGN
...
def _api_call_retry(url, payload, headers, timeout=60, max_attempts=3):
    last_err = None
...
def grok(m):
    ...
    r, err = _api_call_retry(f"{BASE_URL}/chat/completions", p, headers, 60)
    if r:
        return r.json()["choices"][0]["message"]["content"]
    return f"API bleed after 3 retries: {err}"
...
def flux(prompt):
    ...
    r, err = _api_call_retry(f"{BASE_URL}/images/generations", p, headers, 90)
    if r:
        return r.json()["data"][0]["url"]
    return f"Flux denied after 3 retries: {err}"
...
def _evolution_api_call(prompt, temperature=0.7):
    ...
    headers = {"Authorization": f"Bearer {API_KEY}"}
    r, err = _api_call_retry(f"{BASE_URL}/chat/completions", payload, headers, 120)
    if r:
        content = r.json()["choices"][0]["message"]["content"]
        return _strip_fences(content), None
    return None, f"API error after 3 retries: {err}"
...
Do NOT output any code. Only the plan."""
    ...
    headers = {"Authorization": f"Bearer {API_KEY}"}
    r, err = _api_call_retry(f"{BASE_URL}/chat/completions", payload, headers, 90)
    if r:
        return r.json()["choices"][0]["message"]["content"]
    return f"Plan generation failed after 3 retries: {err}"
